...
class CHeadEntity : public CBaseEntity
{
public:
	CHeadEntity() {
		// allocate state
		...
		_objectModel = std::make_unique<Model>("./Resources/Models/head.obj", false);
	...
	void render(const glm::mat4& projection, const glm::mat4& view, glm::vec3 eyePos) override {
		// only render head when it is not this player's head
