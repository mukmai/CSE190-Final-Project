...
std::function<void(int)> playerLeftHandSwitch = [&](int playerID) {
	mtx.lock();
	entityManager->leftHandSwitch(playerID);
	mtx.unlock();
};

std::function<void(int)> playerRightHandSwitch = [&](int playerID) {
	mtx.lock();
	entityManager->rightHandSwitch(playerID);
	mtx.unlock();
};
